...
class _PriceScreenState extends State<PriceScreen> {
  ...
  List<Widget> getCrypticardList() {
    List<Widget> cryptocardList = List<Widget>();

    if (coinData == null) {
      Widget indicator = CircularProgressIndicator();
      cryptocardList.add(Center(child: indicator));
      return cryptocardList;
    }
    for (String cryptoCurrency in cryptoList) {
      Cryptocard cryptocard = Cryptocard(
        value: coinData[cryptoCurrency],
    ...
    return Padding(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Text(
            '1 $cryptoCurrency = ${value == null ? '?' : value.toStringAsFixed(2)} $selectedCurrency',
